@extends('layouts.app')
@section('title', 'The Apostolic Church Nigeria Abuja Metropolis Easter Youth Convocation')

@section('content')
<div class="hero-content">
    <div class="container hero-container">
        <div class="row">
            <div class="col-12 offset-lg-2 col-lg-10">
                <div class="entry-header">
                    <h2>The Unique Youth</h2>
                </div>
                <!-- .entry-header -->

                <div class="mx-auto countdown flex flex-wrap justify-content-around" data-date="2019/04/19">
                    <div class="countdown-holder">
                        <div class="dday">0</div>
                        <label class="countdown-label">Days</label>
                    </div>
                    <!-- .countdown-holder -->

                    <div class="countdown-holder">
                        <div class="dhour">0</div>
                        <label class="countdown-label">Hours</label>
                    </div>
                    <!-- .countdown-holder -->

                    <div class="countdown-holder">
                        <div class="dmin">0</div>
                        <label class="countdown-label">Minutes</label>
                    </div>
                    <!-- .countdown-holder -->

                    <div class="countdown-holder">
                        <div class="dsec">0</div>
                        <label class="countdown-label">Seconds</label>
                    </div>
                    <!-- .countdown-holder -->
                </div>
                <!-- .countdown -->
            </div>
            <!-- .col-12 -->
        </div>
        <!-- row -->

        <div class="entry-footer">
            <a href="/register" class="btn">REGISTER</a>
            {{-- <a href="#" class="btn current">See Lineup</a> --}}

        </div>
    </div>
    <!-- .container -->
    <img class="header-background" src="https://res.cloudinary.com/jabilegbe/image/upload/w_auto,c_scale,g_auto,q_auto/v1551887570/tac/cover_vmba0m.png" alt="">

</div>
<!-- .hero-content -->
@endsection
